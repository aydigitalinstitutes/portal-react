    ...
    const fetchMenuItems = async () => {
      try {
        ...
        if (navRes.data && navRes.data.length > 0) {
          setMenuItems(
            navRes.data.sort((a: MenuItem, b: MenuItem) => a.order - b.order),
          );
        } else {
        ...
        if (themeRes.data && themeRes.data.length > 0) {
          const nameItem = themeRes.data.find(
            (item: any) => item.key === "brand_name",
          );
          const logoItem = themeRes.data.find(
            (item: any) => item.key === "brand_logo",
          );

          setBrand({
            name: nameItem
              ? nameItem.subtitle || nameItem.title
